...
Http.prototype._processMessage = function(stanza) {
    ...
    return this._processIncoming(stanza, data)
}

Http.prototype._processIq = function(stanza) {
    return this._processIncoming(stanza, { type: 'iq', id: stanza.attrs.id })
}
...
Http.prototype.approveRequest = function(data, callback) {
    ...
    if (!this._testRequestParameter(data, type, callback)) return
    if (this.IQ === type) return this._sendIqResponse(data, true)
    return this._sendMessageResponse(data, true)
// message ID 'if'
// message must include original <confirm/>
// message type = normal
...
Http.prototype.denyRequest = function() {
